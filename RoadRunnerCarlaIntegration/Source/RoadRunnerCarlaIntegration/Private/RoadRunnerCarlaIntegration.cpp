...
#include "/home/eloy96vg/carla/Unreal/CarlaUE4/Plugins/Carla/Source/Carla/OpenDrive/OpenDriveActor.h"
// Fix for CARLA 0.9.4
#undef CopyFile
...
void FRoadRunnerCarlaIntegrationModule::RRCarlaPostImport(UFactory* inFactory, UObject* inCreateObject)
{
	...
	FString mapName = FPaths::GetBaseFilename(inFactory->GetCurrentFilename());
	...
	FString filesystemOpenDrivePath = IFileManager::Get().ConvertToAbsolutePathForExternalAppForRead(*projectOpenDrivePath) + "/" + FPaths::GetCleanFilename(rrOpenDriveFile);
